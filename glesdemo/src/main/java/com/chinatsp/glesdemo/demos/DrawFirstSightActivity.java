...
public class DrawFirstSightActivity extends OpenGLESActivity {
    ...
    protected void onStart() {
        ...
        routine = new Routine();
        points = routeMapToCoordinates(path);
...
//        };
        ...
        eyePosition[0] = points[currPosition];
        eyePosition[1] = points[currPosition+1];
        eyePosition[2] = 0.5f;

        target[0] = points[currPosition+3];
        target[1] = points[currPosition+4];
        target[2] = 0;
        ...
        mGlSurfaceView.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                if (currPosition < points.length - 6) {
    ...
    float[] currentDirection = new float[3];
    float[] nextDirection = new float[3];
...
//        double angle = 0;
        if (currPosition == 0) {
            ...
            nextDirection[0] = points[3];
            nextDirection[1] = points[4];
            nextDirection[2] = points[5];

//            angle = getAngle(currentDirection[0], currentDirection[1], nextDirection[0], nextDirection[1]);

        }else if (currPosition > points.length - 6) {
            //倒数第二点

            currentDirection[0] = points[currPosition] - points[currPosition -3];
            currentDirection[1] = points[currPosition +1] - points[currPosition -2];
            currentDirection[2] = points[currPosition+2] - points[currPosition -1];
            ...
            nextDirection[0] = points[currPosition +3] - points[currPosition];
            nextDirection[1] = points[currPosition +4] - points[currPosition +1];
            nextDirection[2] = points[currPosition +5] - points[currPosition +2];
        }
        moveForward(getDistance(currentDirection[0], currentDirection[1]));

        double angle = getAngle(currentDirection[0], currentDirection[1], nextDirection[0], nextDirection[1]);
        if (Math.abs(angle) > Double.MIN_VALUE) {
            turn(angle);
        }
...
//
    ...
    private void moveForward(double distance) {
        ...
        //视线
        double ex = target[0] - eyePosition[0];
        double ey = target[1] - eyePosition[1];
        double ez = 0;

        double len = ex * ex + ey * ey + ez * ez;
        double radio = Math.sqrt(distance * distance / len);

        Log.e("ss", "distance:" +distance+ "   len:" + Math.sqrt(len) + "   radio:"+radio);
        ex *= radio;
        ey *= radio;
        ez *= radio;
...
//        angle *= -1;
    ...
    private void turn(double angle) {
        double diffX = target[0] - eyePosition[0];
        double diffY = target[1] - eyePosition[1];
        double diffZ = 0;
...
//        if (angle >0) {
            ...
            target[0] = (float) (eyePosition[0] + diffX * cos - diffY * sin);
            target[1] = (float) (eyePosition[1] + diffX * sin + diffY * cos);
//        }
    }
