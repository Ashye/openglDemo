...
//    private int currPosition = 154;
    ...
    protected void onStart() {
        ...
        Util.filterDuplicatedPoints(path);
        path = Util.mapToCoordinatesInserted(path);
        ...
        drawRoute.setSaveImage(true);
//        drawRoute.setCarPosition(path.get(0));

        carPosition = path.get(0);
...
//        double positionDis = Util.getDistance(path.get(currPosition)[0], path.get(currPosition)[1], path.get(currPosition+1)[0], path.get(currPosition+1)[1]);
        double carDis = Util.getEarthDistanceBetweenPoints(carPosition, path.get(currPosition+1));

//        Log.e("ss", "current carDis: "+carDis);

            if (carDis < 0.2) {
                currPosition ++;
            }

        List<double[]> pathAtFront = path.subList(currPosition, currPosition+8);
...
//        Log.e("ss", "current:"+currPosition+"/"+path.size()  +" "+0);
    ...
    private void emulateCardMoving(int currPosition) {
        ...
        double[] end = path.get(currPosition+1);

        carPosition[0] += (end[0] - start[0]) / 10;
        carPosition[1] += (end[1] - start[1]) / 10;
    }
